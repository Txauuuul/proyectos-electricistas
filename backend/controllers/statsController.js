...
const statsElectricistas = async (req, res) => {
  try {
    ...
    const stats = electricistas.map((e) => {
      ...
      const ingresos = pagados.reduce((s, p) => s + (p.oferta?.precioTotal || 0), 0);
      const pipeline = mis
        .filter(p => ['offer_sent', 'approved', 'working'].includes(p.estado))
        .reduce((s, p) => s + (p.oferta?.precioTotal || 0), 0);

      const conversionRate = conOferta.length > 0
      ...
      const avgDaysCierre = tiemposCierre.length > 0
      ...
      return {
        ...
        proyectos: mis.map(p => ({
          ...
          estado: p.estado,
          precioTotal: p.oferta?.precioTotal || 0,
    ...
    stats.sort((a, b) => b.stats.ingresosTotales - a.stats.ingresosTotales);
